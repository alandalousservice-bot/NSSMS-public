-- NSSMS-ARCH-012 corrective migration: preserve distinct approval and revocation provenance.

ALTER TABLE qualifications ADD COLUMN revoked_by_user_id uuid REFERENCES users(id) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
BEGIN
  ...
  IF OLD.status <> 'REVOKED' AND NOT (OLD.status='APPROVED' AND NEW.status='REVOKED') AND (NEW.revoked_by_user_id IS NOT NULL OR NEW.revoked_at IS NOT NULL) THEN RAISE EXCEPTION 'revocation provenance may only be set during approval revocation'; END IF;
  ...
  IF OLD.status IN ('APPROVED','REJECTED','REVOKED','ARCHIVED') AND (protected_changed OR NEW.decided_at IS DISTINCT FROM OLD.decided_at OR (OLD.status='REVOKED' AND (NEW.revoked_at IS DISTINCT FROM OLD.revoked_at OR NEW.revoked_by_user_id IS DISTINCT FROM OLD.revoked_by_user_id))) THEN RAISE EXCEPTION 'governed qualification decision context is immutable'; END IF;
  IF NEW.status IN ('APPROVED','REJECTED') AND OLD.status='DRAFT' THEN
    IF NOT EXISTS (SELECT 1 FROM users WHERE id=NEW.decided_by_user_id AND status='ACTIVE') THEN RAISE EXCEPTION 'qualification decision actor must be active'; END IF; NEW.decided_at:=now();
  END IF;
  ...
  IF NEW.status='REVOKED' AND OLD.status='APPROVED' THEN
    IF NEW.revoked_by_user_id IS NULL OR NOT EXISTS (SELECT 1 FROM users WHERE id=NEW.revoked_by_user_id AND status='ACTIVE') THEN RAISE EXCEPTION 'qualification revocation actor must be active'; END IF;
    IF NEW.revoked_at IS NOT NULL THEN RAISE EXCEPTION 'qualification revocation timestamp is database-controlled'; END IF;
    NEW.revoked_at:=now();
  END IF;
