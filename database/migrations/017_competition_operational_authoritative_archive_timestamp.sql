-- NSSMS-ARCH-008 final archive timestamp correction.
-- Archive timestamps are assigned by the database only during a valid archive transition.

CREATE OR REPLACE FUNCTION nssms_protect_competition_stage_history() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE
  identity_context_changed boolean;
...
BEGIN
  ...
  schedule_changed := NEW.start_date IS DISTINCT FROM OLD.start_date OR NEW.end_date IS DISTINCT FROM OLD.end_date;

  IF OLD.status IN ('ACTIVE','RESULTS') AND (identity_context_changed OR schedule_changed) THEN
    RAISE EXCEPTION 'active or results competition stage identity, context, and schedule are immutable';
  END IF;
  IF OLD.status = 'CLOSED' AND NEW.status = 'ARCHIVED' THEN
    IF identity_context_changed OR schedule_changed THEN
    ...
    IF OLD.archived_at IS NOT NULL OR NEW.archived_at IS NOT NULL THEN
      RAISE EXCEPTION 'competition stage archive timestamp is database-controlled';
    END IF;
    NEW.archived_at := now();
  ELSIF OLD.status IN ('CLOSED','ARCHIVED') AND NEW.status = OLD.status AND (
    identity_context_changed OR schedule_changed OR NEW.archived_at IS DISTINCT FROM OLD.archived_at
  ) THEN
  ...
  ELSIF NEW.archived_at IS DISTINCT FROM OLD.archived_at THEN
    RAISE EXCEPTION 'competition stage archival timestamp is controlled by the archive transition';
  END IF;
  RETURN NEW;
END;
$$;
...
BEGIN
  ...
  IF OLD.status = 'COMPLETED' AND NEW.status = 'ARCHIVED' THEN
    ...
    END IF;
    IF OLD.archived_at IS NOT NULL OR NEW.archived_at IS NOT NULL THEN
      RAISE EXCEPTION 'calendar occurrence archive timestamp is database-controlled';
    END IF;
    NEW.archived_at := now();
  ELSIF OLD.status IN ('COMPLETED','ARCHIVED') AND NEW.status = OLD.status AND (
  ...
  ELSIF NEW.archived_at IS DISTINCT FROM OLD.archived_at THEN
    RAISE EXCEPTION 'calendar occurrence archival timestamp is controlled by the archive transition';
  END IF;
  RETURN NEW;
END;
$$;
