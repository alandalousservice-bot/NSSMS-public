...
suite('competition results validation foundation', () => {
  ...
  it('enforces governed result context, lifecycle, revisions, validations, and archival history', async () => {
    const c = await pool!.connect(); try { await c.query('BEGIN');
      ...
      const baseValidation=(await c.query("INSERT INTO result_validations(result_id,revision_no,decision,validator_user_id,notes) VALUES($1,0,'VALIDATED',$2,'base verified') RETURNING id",[result,validator])).rows[0].id;
      expect(baseValidation).toBeTruthy();
      ...
      await fails(c,()=>c.query("INSERT INTO result_revisions(result_id,revision_no,prior_snapshot,new_snapshot,reason) VALUES($1,1,'{\"score\":10}'::jsonb,'{\"score\":12}'::jsonb,'fabricated')",[result]),/prior snapshot/);
      ...
      await fails(c,()=>c.query("INSERT INTO result_validations(result_id,revision_no,decision,validator_user_id) VALUES($1,0,'VALIDATED',$2)",[result,validator]),/latest committed/);
      await fails(c,()=>c.query("INSERT INTO result_revisions(result_id,revision_no,prior_snapshot,new_snapshot,reason) VALUES($1,2,'{\"score\":11}'::jsonb,'{\"score\":13}'::jsonb,'fabricated')",[result]),/prior snapshot/);
      const revision2=(await c.query("INSERT INTO result_revisions(result_id,revision_no,prior_snapshot,new_snapshot,reason) VALUES($1,2,'{\"score\":12}'::jsonb,'{\"score\":13}'::jsonb,'second correction') RETURNING id",[result])).rows[0].id;
      expect(revision2).toBeTruthy();
      await fails(c,()=>c.query("INSERT INTO result_revisions(result_id,revision_no,prior_snapshot,new_snapshot,reason) VALUES($1,4,'{\"score\":13}'::jsonb,'{\"score\":14}'::jsonb,'gap')",[result]),/sequential/);
      ...
      await fails(c,()=>c.query("INSERT INTO result_validations(result_id,revision_no,decision,validator_user_id) VALUES($1,1,'VALIDATED',$2)",[result,validator]),/latest committed/);
      await fails(c,()=>c.query("INSERT INTO result_validations(result_id,revision_no,decision,validator_user_id) VALUES($1,2,'REJECTED',gen_random_uuid())",[result]),/foreign key|active user/);
      const validation=(await c.query("INSERT INTO result_validations(result_id,revision_no,decision,validator_user_id,supersedes_validation_id,notes) VALUES($1,2,'VALIDATED',$2,$3,'corrected verified') RETURNING id",[result,validator,baseValidation])).rows[0].id;
      expect(validation).toBeTruthy();
      await fails(c,()=>c.query("INSERT INTO result_validations(result_id,revision_no,decision,validator_user_id) VALUES($1,2,'VALIDATED',$2)",[result,validator]),/must explicitly supersede/);
      await fails(c,()=>c.query("INSERT INTO result_validations(result_id,revision_no,decision,validator_user_id) VALUES($1,2,'VOID',$2)",[result,validator]),/must explicitly supersede/);
      await fails(c,()=>c.query("INSERT INTO result_validations(result_id,revision_no,decision,validator_user_id) VALUES($1,2,'REJECTED',$2)",[result,validator]),/must explicitly supersede/);
      const voidValidation=(await c.query("INSERT INTO result_validations(result_id,revision_no,decision,validator_user_id,supersedes_validation_id) VALUES($1,2,'VOID',$2,$3) RETURNING id",[result,validator,validation])).rows[0].id;
      expect((await c.query('SELECT governed_status FROM results WHERE id=$1',[result])).rows[0].governed_status).toBe('VOID');
      expect((await c.query("SELECT count(*)::int AS count FROM result_validations prior WHERE prior.result_id=$1 AND prior.decision IN ('VALIDATED','REJECTED','VOID') AND NOT EXISTS (SELECT 1 FROM result_validations successor WHERE successor.supersedes_validation_id=prior.id)",[result])).rows[0].count).toBe(1);
      const revision3=(await c.query("INSERT INTO result_revisions(result_id,revision_no,prior_snapshot,new_snapshot,reason) VALUES($1,3,'{\"score\":13}'::jsonb,'{\"score\":14}'::jsonb,'post-void correction') RETURNING id",[result])).rows[0].id;
      expect(revision3).toBeTruthy();
      await fails(c,()=>c.query("INSERT INTO result_validations(result_id,revision_no,decision,validator_user_id) VALUES($1,3,'VALIDATED',$2)",[result,validator]),/must explicitly supersede/);
      await fails(c,()=>c.query("INSERT INTO result_validations(result_id,revision_no,decision,validator_user_id,supersedes_validation_id) VALUES($1,3,'VALIDATED',$2,$3)",[result,validator,validation]),/must explicitly supersede/);
      const restored=(await c.query("INSERT INTO result_validations(result_id,revision_no,decision,validator_user_id,supersedes_validation_id) VALUES($1,3,'VALIDATED',$2,$3) RETURNING id",[result,validator,voidValidation])).rows[0].id;
      expect((await c.query('SELECT governed_status FROM results WHERE id=$1',[result])).rows[0].governed_status).toBe('VALIDATED');
      await fails(c,()=>c.query("UPDATE result_validations SET notes='tamper' WHERE id=$1",[baseValidation]),/append-only/);
      const official=(await c.query("SELECT CASE WHEN validation.revision_no=0 THEN result.result_data ELSE revision.new_snapshot END AS payload FROM result_validations validation JOIN results result ON result.id=validation.result_id LEFT JOIN result_revisions revision ON revision.result_id=validation.result_id AND revision.revision_no=validation.revision_no WHERE validation.id=$1",[restored])).rows[0];
      expect(official.payload).toEqual({ score: 14 });
      expect((await c.query('SELECT result_data FROM results WHERE id=$1',[result])).rows[0].result_data).toEqual({ score: 11 });
