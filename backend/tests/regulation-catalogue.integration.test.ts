...
suite('regulation catalogue migration', () => {
  ...
  it('enforces sport, event, category, and binding identity', async () => {
    ...
    try {
      ...
      await client.query("INSERT INTO event_rule_bindings(event_id,stage_level_code,regulation_version_id,precedence,effective_period) VALUES($1,'WILAYA',$2,10,'[2096-01-01,2096-07-01)')", [event.rows[0].id, version.rows[0].id]);
      await expectDatabaseFailure(client, () => client.query("INSERT INTO event_rule_bindings(event_id,stage_level_code,regulation_version_id,precedence,effective_period) VALUES($1,'WILAYA',$2,10,'[2096-06-01,2096-12-01)')", [event.rows[0].id, version.rows[0].id]));
      await client.query("INSERT INTO event_rule_bindings(event_id,stage_level_code,regulation_version_id,precedence,effective_period) VALUES($1,'WILAYA',$2,10,'[2096-07-01,2097-01-01)')", [event.rows[0].id, version.rows[0].id]);
      ...
      await client.query("INSERT INTO event_rule_bindings(event_id,stage_level_code,regulation_version_id,precedence,effective_period) VALUES($1,'WILAYA',$2,10,'[2096-01-01,2096-07-01)')", [event.rows[0].id, derivedVersion.rows[0].id]);
