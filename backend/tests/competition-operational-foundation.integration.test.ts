...
suite('competition operational foundation migration', () => {
  ...
  it('enforces occurrence context and venue assignments without global venue locking', async () => {
    ...
    try {
      ...
      await expectDatabaseFailure(client, () => client.query("UPDATE calendar_occurrences SET status='ARCHIVED' WHERE id=$1", [occurrence.rows[0].id]), /immutable/);
      const archivedOccurrence = await client.query("INSERT INTO calendar_occurrences(stage_id,event_id,regulation_version_id,start_at,status) VALUES($1,$2,$3,'2093-02-03T08:00:00Z','ARCHIVED') RETURNING id", [stage.rows[0].id, event.rows[0].id, version.rows[0].id]);
      await expectDatabaseFailure(client, () => client.query("UPDATE calendar_occurrences SET status='DRAFT' WHERE id=$1", [archivedOccurrence.rows[0].id]), /immutable/);
