...
export type Filters = Record<string, string | boolean | undefined>;

async function list(select: string, from: string, scope: Scope, filters: Filters, fields: Record<string, string>, page: Page, order: string, predicates: (filters: Filters) => string[] = () => []) {
  const values: unknown[] = [...scope.values];
  const where = [scope.sql];
  for (const [filter, column] of Object.entries(fields)) {
    const filterValue = filters[filter];
    if (filterValue !== undefined) { values.push(filterValue); where.push(`${column}=$${values.length}`); }
  }
  where.push(...predicates(filters));
...
export const competitionCollections = {
  ...
  results: (scope: Scope, filters: Filters, page: Page) => {
    const latestRevision = `coalesce((select max(result_revisions.revision_no) from result_revisions where result_revisions.result_id=r.id),0)`;
    const evidenceCandidate = `(v.decision='VALIDATED' and r.governed_status='VALIDATED' and v.revision_no=${latestRevision})`;
    return list(`r.*, v.id as validation_id, v.decision, v.revision_no, v.validator_user_id, v.created_at as validation_created_at, case when v.revision_no=0 then r.result_data else revision.new_snapshot end as official_payload, ${latestRevision} as latest_revision_no, ${evidenceCandidate} as is_evidence_candidate`, `results r left join competition_entries e on e.id=r.competition_entry_id left join participants legacy_participant on legacy_participant.id=r.participant_id left join educational_institutions i on i.id=coalesce(e.institution_id,legacy_participant.institution_id) left join lateral (select current_validation.* from result_validations current_validation where current_validation.result_id=r.id and not exists(select 1 from result_validations superseding where superseding.supersedes_validation_id=current_validation.id) order by current_validation.created_at desc,current_validation.id desc limit 1) v on true left join result_revisions revision on revision.result_id=r.id and revision.revision_no=v.revision_no`, scope, filters, { stage_id: 'r.stage_id', occurrence_id: 'r.occurrence_id', event_id: 'r.event_id', category_id: 'r.category_id', competition_entry_id: 'r.competition_entry_id', regulation_version_id: 'r.regulation_version_id', status: 'r.governed_status' }, page, 'r.created_at desc, r.id desc', query => query.evidence_candidate === true ? [evidenceCandidate] : []);
  },
